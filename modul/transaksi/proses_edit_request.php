...
if ($_POST) {
    ...
    if (mysqli_query($koneksi, $query_h)) {
        ...
        foreach ($nama_barang as $key => $val) {
            ...
            // --- PERBAIKAN LOGIKA: CEK APAKAH BARANG ADA DI MASTER ---
            ...
            if ($data_b) {
                // Jika ditemukan di master
                $id_barang = $data_b['id_barang'];
                $nama_manual = ""; // Kosongkan manual karena sudah ada di master
            } else {
                // Jika tidak ditemukan (Barang Manual/Besar)
                $id_barang = 0;
                $nama_manual = $nama;
            }
            // -------------------------------------------------------
